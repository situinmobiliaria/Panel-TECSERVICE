...
function initAlertas(){
  ...
  if(filtDiv){
    ...
    bAll.className='btn on';bAll.dataset.altc='todas';bAll.textContent='Todos';
...
function renderAlertas(){
  ...
  tbody.innerHTML=rows.map(r=>{
    ...
    // Si cuota_uf ≈ neta_mes significa que la columna Z tiene el valor en CLP, no en UF
    const cuotaEsCLP=r.cuota_uf>0&&Math.abs(r.cuota_uf-r.neta_mes)<=1;
    const cuotaTxt=r.cuota_uf>0&&!cuotaEsCLP?r.cuota_uf.toFixed(2)+' UF':'—';
    const cuotaTitle=cuotaEsCLP?'title="Cuota no definida en UF, valor en CLP"':'';
    ...
    return`<tr>
      ...
      <td class="num" style="font-size:.62rem;color:${cuotaEsCLP?'var(--mut)':'inherit'}" ${cuotaTitle}>${cuotaTxt}</td>
