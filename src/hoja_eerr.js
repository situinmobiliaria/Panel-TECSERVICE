...
async function eerrExportPDF() {
  ...
  function cleanClone(el) {
    ...
    c.querySelectorAll('td,th').forEach(n => {
      ...
      n.style.padding    = '2px 3px';
      n.style.lineHeight = '1.15';
  ...
  // 1. Measure natural scroll widths of each table so we know how wide the container needs to be
  const eerrW  = eerrTbl  ? eerrTbl.scrollWidth  : 0;
  const ratioW = ratioTbl ? ratioTbl.scrollWidth : 0;
  const contentW = Math.max(eerrW, ratioW, 900);

  // 2. Build off-screen container at exact content width
  const wrap = document.createElement('div');
  wrap.style.cssText = `position:fixed;left:-${contentW + 200}px;top:0;background:#fff;` +
    `padding:10px 16px 16px;font-family:Arial,sans-serif;color:#111;` +
    `width:${contentW + 32}px;box-sizing:border-box`;

  // Header
  const hdr = document.createElement('div');
  hdr.style.cssText = 'margin-bottom:7px;border-bottom:2px solid #002D73;padding-bottom:4px;display:flex;align-items:baseline;gap:12px';
  hdr.innerHTML = `<span style="font-size:10px;font-weight:700;color:#002D73">TECSERVICE — Estado de Resultado y Análisis de Ratios</span>` +
    `<span style="font-size:8px;color:#666">Período: ${mesLbl} &nbsp;·&nbsp; Cifras en MM$</span>`;
  ...
  function makeSection(label, srcEl) {
    ...
    lbl.style.cssText = 'font-size:7.5px;font-weight:700;color:#002D73;letter-spacing:.06em;text-transform:uppercase;margin-bottom:3px';
  ...
  try {
    // 3. Capture the full content at scale 1.5 (enough DPI without giant file)
    ...
    // 4. Scale to fit A4 landscape (297 × 210 mm) with 6 mm margins
    const margin  = 6;
    const usableW = 297 - margin * 2;
    const usableH = 210 - margin * 2;
    const px2mm   = 25.4 / (96 * 1.5);
