...
def build_app_data(contratos, panel_raw, bbdd, visitas, satisf, mes_corte, analisis_fac=None, base_instalada=None):
    ...
    # "Ingresos No Recurrentes" cubre TODA la facturación TS en BBDD, sin distinción.
    # La separación real contr vs no-contr viene de la columna contr_2026 de FACTURACIÓN.
    _total_ytd_fac = sum(p.get("real_ytd", 0) for p in panel_raw)
    _contr_ytd_fac = sum(p.get("presup_contr_ytd", 0) for p in panel_raw)  # col 6 = contr_2026
