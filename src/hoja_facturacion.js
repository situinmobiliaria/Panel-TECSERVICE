...
function renderFcVsPptoMensual(){
  ...
  const m=APP_DATA.mensual||{};
  // Real mensual total (Facturas ST/REAS/Traz, misma fuente que KPIs)
  const realTotMes=(m.facturado&&m.facturado['2026'])||Array(12).fill(0);
  ...
  // Otras facturaciones = total − contratos (mínimo 0)
  const realOtrosMes=realTotMes.map((v,i)=>Math.max(0,v-(realConMes[i]||0)));
  // Ppto contratos = 50% ppto; Ppto otras = 50% ppto
  const pptoConMes=pptoMes.map(v=>v*0.5);
  const pptoOtrosMes=pptoMes.map(v=>v*0.5);
  // Acumulados
  const acum=arr=>arr.reduce((a,v,i)=>{a.push((a[i-1]||0)+v);return a;},[]);
  const realConAcum=acum(realConMes),pptoConAcum=acum(pptoConMes);
  const realOtrosAcum=acum(realOtrosMes),pptoOtrosAcum=acum(pptoOtrosMes);

  const barConColor=(i)=>i<MES_CORTE?'rgba(40,210,195,.78)':'rgba(184,193,216,.28)';
  const barOtrColor=(i)=>i<MES_CORTE?'rgba(212,96,0,.72)':'rgba(184,193,216,.28)';
  const chartOpts=()=>({
  ...
  const mkDatasets=(real,ppto,realAc,pptoAc,barFn,lineColor,lineAcumColor)=>[
    ...
    {label:'Acum. Real',data:realAc.map(v=>v/1e6),type:'line',borderColor:lineAcumColor||C.te,backgroundColor:'rgba(40,210,195,.07)',borderWidth:2.5,pointRadius:0,tension:0.4,fill:false,yAxisID:'yA',order:1},
  ...
  if(ctxT){if(_chFcVspTotMen)_chFcVspTotMen.destroy();
    _chFcVspTotMen=new Chart(ctxT.getContext('2d'),{type:'bar',
      data:{labels:MESES_ABR,datasets:mkDatasets(realConMes,pptoConMes,realConAcum,pptoConAcum,barConColor,C.te,C.te)},
      options:chartOpts()});}

  // Gráfico 2: Otras Facturaciones
  ...
  if(ctxC){if(_chFcVspConMen)_chFcVspConMen.destroy();
    _chFcVspConMen=new Chart(ctxC.getContext('2d'),{type:'bar',
      data:{labels:MESES_ABR,datasets:mkDatasets(realOtrosMes,pptoOtrosMes,realOtrosAcum,pptoOtrosAcum,barOtrColor,C.or,C.or)},
      options:chartOpts()});}
}
