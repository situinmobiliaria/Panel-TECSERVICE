...
function renderFcVsPptoMensual(){
  ...
  const mkDatasets=(real,ppto,barFn)=>[
    {label:'Real mensual',data:real.map(v=>v/1e6),backgroundColor:real.map((_,i)=>barFn(i)),borderRadius:4,order:2},
    {label:'Ppto mensual',data:ppto.map(v=>v/1e6),type:'line',borderColor:'#FFC000',backgroundColor:'transparent',borderWidth:2,borderDash:[5,3],pointRadius:3,pointBackgroundColor:'#FFC000',tension:0.3,order:1}
  ];
  ...
  if(ctxT){if(_chFcVspTotMen)_chFcVspTotMen.destroy();
    _chFcVspTotMen=new Chart(ctxT.getContext('2d'),{type:'bar',
      data:{labels:MESES_ABR,datasets:mkDatasets(realConMes,pptoConMes,barConColor)},
      options:chartOpts()});}
  ...
  if(ctxC){if(_chFcVspConMen)_chFcVspConMen.destroy();
    _chFcVspConMen=new Chart(ctxC.getContext('2d'),{type:'bar',
      data:{labels:MESES_ABR,datasets:mkDatasets(realOtrosMes,pptoOtrosMes,barOtrColor)},
      options:chartOpts()});}
}
